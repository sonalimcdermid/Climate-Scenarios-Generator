...
infile    <- c('INCO0XXX','INCOIEXF','INCOIIXF','INCOIKXF','INCOIOXF','INCOIRXF')
...
jd.start <- c(274,274,274,274,274,274)
jd.end   <- c(365,365,365,365,365,365)
...
###    'sum'                 - the sum of the selected climate variable
...
source(paste(rootDir, 'r/acr_agmipload.R', sep=''))
...
tmp     <- matrix(NaN,31,length(infile)+1)
metric[,1]  <- 1980:2010
...
for (ii in 1:length(infile)) {
  ...
  tmp <- acr_agmip2metrics(infile[ii], jd.start[ii], jd.end[ii], clim.var, analysis.type, reference, special.operator)
  metric[,ii+1]=tmp[,2]
