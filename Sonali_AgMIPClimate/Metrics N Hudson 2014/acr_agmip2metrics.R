...
#
#  THIS WAS FORMERLY acr_agmip2metrics.R --  September 30, 2013 by Nicholas Hudson
#
#     Author: Alex Ruane
...
# 	specialoperator <- -1
  ...
  ##  Read in .AgMIP file
  climdatainfo  <- read.table(paste(rootDir, 'data/Climate/Historical/', infile, '.AgMIP', sep=''), skip=3, nrows=1)
	climdata      <- read.table(paste(rootDir, 'data/Climate/Historical/', infile, '.AgMIP', sep=''), skip=5, sep='')
  ...
  for (yyyy in years) {
    ...
    if (analysistype == 'mean') metric[yyyy-1979] <- mean(climdata[firstday:lastday, column])
    if (analysistype == 'sum') metric[yyyy-1979] <- sum(climdata[firstday:lastday, column])
